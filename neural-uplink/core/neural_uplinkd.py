...
class Task:
    def __init__(self, task_id: str, goal: str, context: dict, constraints: dict, callback_url: str = ""):
...
def get_agents_by_capability(capability: str) -> list:
    capability_map = {
        ...
        "data": "agent-data",
        "ops": "agent-ops",
        "world": "agent-worldbuilder",
    }
    target = capability_map.get(capability, "agent-dialog")
    return [aid for aid, info in AGENT_REGISTRY.items() if info["name"] == target]

def route_task_to_agent(task: Task) -> Optional[str]:
    ...
    elif any(w in goal for w in ["chat", "talk", "ask", "explain", "write", "code", "llm", "respond"]):
...
async def forward_to_agent(agent_id: str, task: Task) -> dict:
    ...
    payload = {
        ...
        "callback_url": f"http://localhost:7000/callback/{task.task_id}"
    }
    try:
...
async def callback_handler(request):
    ...
    if task_id in TASKS:
        TASKS[task_id].status = data.get("status", "completed")
        TASKS[task_id].result = data.get("result", {})
        TASKS[task_id].updated_at = datetime.utcnow().isoformat()
        log.info(f"Task {task_id} callback: {data.get('status')}")
...
async def submit_task(request):
    ...
    TASKS[task_id] = task
    log.info(f"Task submitted: {task_id} — {task.goal[:60]}")
    ...
    if agent_id:
        ...
        return web.json_response({"task_id": task_id, "status": "no_agent_available", "message": "No agent capable of this task"})
...
async def shutdown_node(request):
    ...
    return web.json_response({"ok": True, "message": "Node shutdown initiated"})
